...
void initCarro(Car *c){
	c->x = 500;
	c->y = SCREEN_H - CAR_H;
	c->cor = al_map_rgb(225,32,65);
}
...
void draw_car(Car c){
	al_draw_filled_rectangle(c.x, c.y, c.x + CAR_H, c.y + CAR_W, c.cor);
}
...
void processaTeclaNaveg(Car *c, int tecla){
	...
	switch(tecla) {
		...
		case ALLEGRO_KEY_LEFT:
			if(c->x - PASSO >= CAR_W)c->x -= PASSO;
			//if(c->x + PASSO + CAR_W <= SCREEN_W)c->x -= PASSO;
			break;		
...
//void processaTeclaNaveg(Car *c, int tecla){
	...
	if(ev.type == ALLEGRO_EVENT_KEY_DOWN) {
		...
		processaTeclaNaveg(&carro, ev.keyboard.keycode);
